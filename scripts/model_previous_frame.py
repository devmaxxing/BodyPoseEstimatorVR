...
model = Sequential()
model.add(Dense(11, input_shape = (24,)))
...
inputDataTrain = array(p.ParseWithSpine(dataFileTrain))
...
print(estimate[0][1])
print(estimate[0][2])
tolist.append(estimate[0][0])###########################use estimator to get first frame prev.
tolist.append(estimate[0][1])
tolist.append(estimate[0][2])

wrap = [tolist]
print(wrap)
predictedFrame = model.predict(array(wrap), 1)

test.append(predictedFrame[0])
for i in range(1, len(inputDataTest)):
    testFrame = list(inputDataTest[i])
    testFrame.append(predictedFrame[0][0])
    testFrame.append(predictedFrame[0][1])
    testFrame.append(predictedFrame[0][2])
    wrap = [testFrame]
